...
function packageDoctor() {
  ...
  const phaseKRehearsalEvidenceDigest = readText(packageFiles.phaseKRehearsalEvidenceDigest, errors);
  if (phaseKRehearsalEvidenceDigest) {
    ...
    check(/pr-shepherd-rehearsal-evidence-digest\/v1/.test(phaseKRehearsalEvidenceDigest), errors, 'Phase K runbook must document the digest schema');
    ...
    check(/candidateAllowed=true/.test(phaseKRehearsalEvidenceDigest) && /does not approve live repair|does not authorize live repair/i.test(phaseKRehearsalEvidenceDigest), errors, 'Phase K runbook must keep the candidate gate separate from live approval');
