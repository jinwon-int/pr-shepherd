...
const SECRET_FIELD_NAMES = new Set([
  'token', 'secret', 'password', 'authorization', 'proxyauthorization',
  'accesstoken', 'refreshtoken', 'idtoken', 'clientsecret', 'apikey',
  'githubtoken', 'githubpat',
]);

function isSecretField(key) {
  return SECRET_FIELD_NAMES.has(key.toLowerCase().replace(/[_-]/g, ''));
}

export function redact(text) {
  return String(text ?? '')
    .replace(/(?:gh[pousr]_|github_pat_)[A-Za-z0-9_]+/g, '[REDACTED_GITHUB_TOKEN]')
    .replace(/(\b(?:token|secret|password|(?:proxy[-_]?)?authorization|(?:access|refresh|id)[-_]?token|client[-_]?secret|api[-_]?key|github[-_]?(?:token|pat))\b["']?\s*[=:]\s*)("(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'|(?:(?:bearer|basic|token)\s+)?[^\s,;}"']+)/ig,
      (_match, prefix, value) => {
...
export function redactLedgerValue(value, target = {}) {
  ...
  if (isPlainObject(value)) {
    return Object.fromEntries(Object.entries(value)
      ...
      .map(([key, item]) => [key, isSecretField(key) ? '[REDACTED]' : redactLedgerValue(item, target)]));
  }
  return value;
}
