...
import { AUTOMATIC_ACTION_CLASSES, BOUNDED_RETRY_SCOPE, DEFAULT_SITUATION_REPORT_EVERY_MS, GITHUB_PROVIDERS, MAX_BOUNDED_RETRY_ATTEMPTS, MINOR_AUTO_MERGE_SCOPE, MINOR_AUTO_ROLLOUT_MODES, MINOR_AUTO_SAFE_REPAIR_SCOPE, MIN_LIVE_OPENCLAW_SITUATION_REPORT_EVERY_MS, RISKY_CHANGE_APPROVAL_SCOPE, RISK_CLASS_SEVERITY, SUPPORTED_MERGE_METHODS, SUPPORTED_MINOR_AUTO_SAFE_RESOLVERS, findOpenClawRuntimeContextPaths } from './policy.mjs';
...
// `https://user:pass@host` detection without a regex over the whole value.
// The original `[^/\s:@]+:[^@\s]+@` backtracked polynomially on long
// `a:b:c:...` input, and a simpler `https?://([^\s@]+)@` still rescans to the
// end from every `http://` start (CodeQL js/polynomial-redos both times).
// Per whitespace token: locate the scheme, take up to the first `@`, and
// apply the same acceptance as before — userinfo present, contains `:`,
// contains no `/`. indexOf/slice only, so linear in the input.
export function urlEmbedsUserinfoWithPassword(value) {
  for (const token of String(value).split(/\s+/)) {
    const lower = token.toLowerCase();
    let at = lower.indexOf('http://');
    let schemeLength = 7;
    if (at < 0) { at = lower.indexOf('https://'); schemeLength = 8; }
    if (at < 0) continue;
    const rest = token.slice(at + schemeLength);
    const end = rest.indexOf('@');
    if (end <= 0) continue;
    const userinfo = rest.slice(0, end);
    if (!userinfo.includes('/') && userinfo.includes(':')) return true;
  }
  return false;
}

// Branch names reach `git fetch/rev-parse/checkout/ls-remote` as positional
// arguments (pr-shepherd.mjs handleRepair). A name that starts with `-` is
// parsed by git as an option — `--upload-pack=<cmd>` runs an arbitrary
// command (CodeQL js/second-order-command-line-injection) — so anything git
// itself would refuse (`git check-ref-format --branch`) or that could be
// mistaken for an option/refspec trick is rejected before it is used.
const UNSAFE_BRANCH_CHARS = /[\s\x00-\x1f\x7f~^:?*[\\]/;
export function isSafeGitBranchName(name) {
  ...
  if (name.endsWith('.') || name.endsWith('.lock') || name.includes('/.') || name.startsWith('.')) return false;
  return !UNSAFE_BRANCH_CHARS.test(name);
}

export function validateGitBranchName(errors, target, targetPath, field) {
  const value = target[field];
  if (typeof value !== 'string' || value.trim() === '') return; // validateRequiredString reports this
  if (!isSafeGitBranchName(value)) errors.push(`${targetPath}.${field} is not a safe git branch name`);
}

export function assertSafeGitBranchName(name, label = 'branch') {
  if (!isSafeGitBranchName(name)) throw new Error(`refusing to pass unsafe ${label} to git: ${JSON.stringify(String(name)).slice(0, 80)}`);
  return name;
}
...
export function secretLookingValues(value, path = 'config', out = []) {
  if (typeof value === 'string') {
    ...
    const looksLikeSecret = /gh[pousr]_[A-Za-z0-9_]{20,}/.test(value)
      || urlEmbedsUserinfoWithPassword(value)
...
export function validateAutomaticActions(errors, target, targetPath) {
  ...
  validateAutoMerge(errors, target, targetPath);
  validateBoundedRetry(errors, target, targetPath);
  validateRiskyChangeApproval(errors, target, targetPath);
...
export function validateAutoMerge(errors, target, targetPath) {
  const policy = target.automaticActions?.autoMerge;
  if (policy === undefined) return;
  const policyPath = `${targetPath}.automaticActions.autoMerge`;
  if (!isPlainObject(policy)) {
    errors.push(`${policyPath} must be an object`);
    return;
  }
  if (policy.enabled !== undefined && typeof policy.enabled !== 'boolean') errors.push(`${policyPath}.enabled must be a boolean`);
  if (policy.scope !== undefined && policy.scope !== MINOR_AUTO_MERGE_SCOPE) errors.push(`${policyPath}.scope must be ${MINOR_AUTO_MERGE_SCOPE}`);
  if (policy.mergeMethod !== undefined && !SUPPORTED_MERGE_METHODS.includes(policy.mergeMethod)) errors.push(`${policyPath}.mergeMethod must be one of: ${SUPPORTED_MERGE_METHODS.join(', ')}`);
  const pathAllowlist = Array.isArray(policy.pathAllowlist) ? policy.pathAllowlist.filter((item) => typeof item === 'string' && item.trim() !== '') : [];
  if (policy.enabled === true) {
    if (policy.scope !== MINOR_AUTO_MERGE_SCOPE) errors.push(`${policyPath}.scope must be ${MINOR_AUTO_MERGE_SCOPE} when enabled`);
    if (!SUPPORTED_MERGE_METHODS.includes(policy.mergeMethod)) errors.push(`${policyPath}.mergeMethod is required and must be one of: ${SUPPORTED_MERGE_METHODS.join(', ')} when enabled`);
    if (typeof policy.targetBranch !== 'string' || policy.targetBranch.trim() === '') errors.push(`${policyPath}.targetBranch is required when enabled`);
    if (!Array.isArray(policy.requiredChecks) || policy.requiredChecks.filter((item) => typeof item === 'string' && item.trim() !== '').length === 0) errors.push(`${policyPath}.requiredChecks must be a non-empty string array when enabled`);
    if (pathAllowlist.length === 0) errors.push(`${policyPath}.pathAllowlist must be a non-empty string array when enabled`);
    if (!Array.isArray(policy.resolverAllowlist) || policy.resolverAllowlist.filter((item) => typeof item === 'string' && item.trim() !== '').length === 0) errors.push(`${policyPath}.resolverAllowlist must be a non-empty string array when enabled`);
    if (!Array.isArray(policy.branchAllowlist) || policy.branchAllowlist.filter((item) => typeof item === 'string' && item.trim() !== '').length === 0) errors.push(`${policyPath}.branchAllowlist must be a non-empty string array when enabled`);
  }
  pathAllowlist.forEach((path, index) => {
    validateConflictPolicyPath(errors, `${policyPath}.pathAllowlist[${index}]`, path);
    const risk = minorAutoPathRiskReason(path.endsWith('/**') ? `${path.slice(0, -3)}/README.md` : path);
    if (risk) errors.push(`${policyPath}.pathAllowlist[${index}] is not minor-safe for auto-merge: ${risk}`);
  });
  (Array.isArray(policy.resolverAllowlist) ? policy.resolverAllowlist : []).forEach((resolver, index) => {
    if (typeof resolver === 'string' && resolver.trim() && !SUPPORTED_MINOR_AUTO_SAFE_RESOLVERS.includes(resolver)) errors.push(`${policyPath}.resolverAllowlist[${index}] is not a supported minor-auto-safe resolver: ${resolver}`);
  });
}

export function validateBoundedRetry(errors, target, targetPath) {
  const policy = target.automaticActions?.boundedRetry;
  if (policy === undefined) return;
  const policyPath = `${targetPath}.automaticActions.boundedRetry`;
  if (!isPlainObject(policy)) {
    errors.push(`${policyPath} must be an object`);
    return;
  }
  if (policy.enabled !== undefined && typeof policy.enabled !== 'boolean') errors.push(`${policyPath}.enabled must be a boolean`);
  if (policy.scope !== undefined && policy.scope !== BOUNDED_RETRY_SCOPE) errors.push(`${policyPath}.scope must be ${BOUNDED_RETRY_SCOPE}`);
  if (policy.maxAttempts !== undefined && (!Number.isInteger(Number(policy.maxAttempts)) || Number(policy.maxAttempts) < 1 || Number(policy.maxAttempts) > MAX_BOUNDED_RETRY_ATTEMPTS)) errors.push(`${policyPath}.maxAttempts must be an integer between 1 and ${MAX_BOUNDED_RETRY_ATTEMPTS}`);
  if (policy.budgetPerDay !== undefined && (!Number.isFinite(Number(policy.budgetPerDay)) || Number(policy.budgetPerDay) <= 0)) errors.push(`${policyPath}.budgetPerDay must be a positive number`);
  if (policy.enabled === true) {
    if (policy.scope !== BOUNDED_RETRY_SCOPE) errors.push(`${policyPath}.scope must be ${BOUNDED_RETRY_SCOPE} when enabled`);
    if (!Number.isInteger(Number(policy.maxAttempts)) || Number(policy.maxAttempts) < 1 || Number(policy.maxAttempts) > MAX_BOUNDED_RETRY_ATTEMPTS) errors.push(`${policyPath}.maxAttempts is required and must be an integer between 1 and ${MAX_BOUNDED_RETRY_ATTEMPTS} when enabled`);
    if (!Number.isFinite(Number(policy.budgetPerDay)) || Number(policy.budgetPerDay) <= 0) errors.push(`${policyPath}.budgetPerDay is required and must be a positive number when enabled`);
  }
}

export function validateRiskyChangeApproval(errors, target, targetPath) {
  const policy = target.automaticActions?.riskyChangeApproval;
  if (policy === undefined) return;
  const policyPath = `${targetPath}.automaticActions.riskyChangeApproval`;
  if (!isPlainObject(policy)) {
    errors.push(`${policyPath} must be an object`);
    return;
  }
  if (policy.enabled !== undefined && typeof policy.enabled !== 'boolean') errors.push(`${policyPath}.enabled must be a boolean`);
  if (policy.scope !== undefined && policy.scope !== RISKY_CHANGE_APPROVAL_SCOPE) errors.push(`${policyPath}.scope must be ${RISKY_CHANGE_APPROVAL_SCOPE}`);
  if (policy.expiresAt !== undefined && (typeof policy.expiresAt !== 'string' || Number.isNaN(Date.parse(policy.expiresAt)))) errors.push(`${policyPath}.expiresAt must be an ISO-8601 timestamp`);
  if (policy.approvedAt !== undefined && (typeof policy.approvedAt !== 'string' || Number.isNaN(Date.parse(policy.approvedAt)))) errors.push(`${policyPath}.approvedAt must be an ISO-8601 timestamp`);
  if (policy.riskClass !== undefined && !RISK_CLASS_SEVERITY.includes(policy.riskClass)) errors.push(`${policyPath}.riskClass must be one of: ${RISK_CLASS_SEVERITY.join(', ')}`);
  if (policy.enabled === true) {
    if (policy.scope !== RISKY_CHANGE_APPROVAL_SCOPE) errors.push(`${policyPath}.scope must be ${RISKY_CHANGE_APPROVAL_SCOPE} when enabled`);
    if (typeof policy.approvalId !== 'string' || policy.approvalId.trim() === '') errors.push(`${policyPath}.approvalId is required when enabled`);
    if (typeof policy.expiresAt !== 'string' || Number.isNaN(Date.parse(policy.expiresAt))) errors.push(`${policyPath}.expiresAt is required and must be an ISO-8601 timestamp when enabled (risky push must expire)`);
    if (typeof policy.approvedBy !== 'string' || policy.approvedBy.trim() === '') errors.push(`${policyPath}.approvedBy is required when enabled`);
    if (!Array.isArray(policy.branchAllowlist) || policy.branchAllowlist.filter((item) => typeof item === 'string' && item.trim() !== '').length === 0) errors.push(`${policyPath}.branchAllowlist must be a non-empty string array when enabled`);
  }
}
...
export function validateConfigObject(cfg, configPath = null) {
  ...
  cfg.targets.forEach((target, index) => {
    ...
    if (isEnabledTarget(target)) {
      ...
      validateGitBranchName(errors, target, targetPath, 'headBranch');
      validateGitBranchName(errors, target, targetPath, 'baseBranch');
      ...
      if (typeof target.lockPath === 'string' && target.lockPath.trim()) {
        ...
        if (/^\/(?:var\/)?tmp\//.test(key)) {
          warnings.push(`${targetPath}.lockPath is under a world-writable shared directory (${key}); on multi-user hosts another local user can squat or replace the lock, and tmp cleanup can break stale-lock handling. Prefer a state-adjacent path such as ${dirname(String(target.statePath || '<state-root>/state.json'))}/${basename(key)}`);
        }
      }
    }

    if (target.github !== undefined) {
      if (!isPlainObject(target.github)) errors.push(`${targetPath}.github must be an object`);
      else {
        if (target.github.provider !== undefined && !GITHUB_PROVIDERS.includes(target.github.provider)) {
          errors.push(`${targetPath}.github.provider must be one of [${GITHUB_PROVIDERS.join(', ')}]`);
        }
        if (target.github.apiBaseUrl !== undefined && !/^https:\/\//.test(String(target.github.apiBaseUrl))) {
          errors.push(`${targetPath}.github.apiBaseUrl must be an https URL`);
        }
        if (target.github.retryDelaysMs !== undefined
          && (!Array.isArray(target.github.retryDelaysMs) || !target.github.retryDelaysMs.every((value) => Number.isFinite(Number(value)) && Number(value) >= 0))) {
          errors.push(`${targetPath}.github.retryDelaysMs must be an array of non-negative numbers`);
        }
      }
    }
