...
// `https://user:pass@host` detection. The previous single regex
// (`[^/\s:@]+:[^@\s]+@`) let the second class re-consume `:` and backtracked
// polynomially on long `a:b:c:...` inputs (CodeQL js/polynomial-redos). One
// unambiguous match for the userinfo, then plain string checks.
export function urlEmbedsUserinfoWithPassword(value) {
  const m = /https?:\/\/([^\s@]+)@/i.exec(String(value));
  if (!m) return false;
  const userinfo = m[1];
