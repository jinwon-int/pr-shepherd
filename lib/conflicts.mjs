...
import { getResolver } from './resolvers.mjs';
...
function applyAutoSafeResolver(target, policyEntry) {
  const resolver = policyEntry ? getResolver(policyEntry.resolver) : null;
  if (!resolver || typeof resolver.resolve !== 'function') return false;
  if (!resolver.resolve(target, policyEntry)) return false;
  run('git', ['add', policyEntry.path], { cwd: target.worktreePath });
  return true;
}
...
export function resolveChangelogConflict(target, policyEntry = null) {
  ...
  return applyAutoSafeResolver(target, cfg);
}
...
export function resolveAutoSafeConflicts(target, conflictInfo) {
  for (const entry of conflictInfo.entries) {
    ...
    if (!applyAutoSafeResolver(target, entry.policy)) return false;
  }
  return true;
}
